...
export type WorkspaceMutationErrorKind =
  | "free-limit"
  | "invalid-date"
  | "invalid-input"
  | "not-found"
...
export class WorkspaceMutationError extends Error {
  constructor(
    message: string,
    public readonly kind: WorkspaceMutationErrorKind,
    cause: unknown,
  ) {
    super(message, { cause });
...
const freeLimitMessages: Record<string, string> = {
...
export function workspaceMutationError(error: unknown): Error {
  ...
  const raw = `${backend.message ?? ""} ${backend.details ?? ""}`;
  const text = raw.toLowerCase();

  if (
    backend.message === "STUDY_ACTIVE_SESSION_CONFLICT" ||
    raw.includes("study_sessions_one_active_per_user")
  ) {
  ...
  if (backend.message === "FREE_CREATION_LIMIT_REACHED") {
    const resource = Object.keys(freeLimitMessages).find(
      (key) =>
        backend.details?.includes(`"resource": "${key}"`) ||
        backend.details?.includes(`"resource":"${key}"`),
    );
    return new WorkspaceMutationError(
      resource ? freeLimitMessages[resource] : "Você atingiu o limite do plano gratuito.",
      "free-limit",
      error,
    );
  }
  ...
  if (backend.code === "23503" || text.includes("foreign key constraint")) {
    return new WorkspaceMutationError(
      "Este item ainda possui dados relacionados e não pode ser excluído agora.",
      "delete-restricted",
      error,
    );
  }
  ...
  if (text.includes("stale")) {
    return new WorkspaceMutationError(
      "Este item mudou em outro lugar. Atualize a tela antes de tentar novamente.",
      "stale",
      error,
    );
  }

  if (text.includes("not found") || text.includes("não encontrada") || text.includes("_not_found")) {
