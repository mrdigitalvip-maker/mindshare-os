...
const CREATOR_YOUTUBE_ERROR_CODES = new Set<CreatorYouTubeMetadataErrorCode>([
  "configuration_error",
  "origin_not_allowed",
  "unauthorized",
  "invalid_youtube_url",
...
function isCreatorYouTubeErrorCode(value: unknown): value is CreatorYouTubeMetadataErrorCode {
  return typeof value === "string" && CREATOR_YOUTUBE_ERROR_CODES.has(value as CreatorYouTubeMetadataErrorCode);
}

async function creatorYouTubeInvokeError(error: unknown): Promise<CreatorYouTubeMetadataError> {
  const context =
  ...
  if (context && typeof context === "object" && "clone" in context) {
    try {
      ...
      if (isCreatorYouTubeErrorCode(code)) return new CreatorYouTubeMetadataError(code);
    } catch {
      // Fall through to the safe generic code. Never expose raw provider responses.
    }
  }

  const message = error instanceof Error ? error.message : "";
  const code = [...CREATOR_YOUTUBE_ERROR_CODES].find((candidate) => message.includes(candidate));
...
export async function inspectCreatorYouTubeUrl(url: string): Promise<CreatorYouTubeMetadata> {
  ...
  const { data, error } = await supabase.functions.invoke<
    CreatorYouTubeMetadata | { error?: { code?: string } }
  >("creator-youtube-metadata", { body: { url: value } });

  if (error) throw await creatorYouTubeInvokeError(error);
  if (!data || ("error" in data && data.error)) {
    const code = data && "error" in data ? data.error?.code : undefined;
    throw new CreatorYouTubeMetadataError(
      isCreatorYouTubeErrorCode(code) ? code : "youtube_metadata_unavailable",
    );
  }
  if (
    !("videoId" in data) ||
