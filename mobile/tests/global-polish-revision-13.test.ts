...
describe("Revision 13 — acabamento global nativo", () => {
  ...
  test("nomes técnicos de rotas não são apresentados como títulos", () => {
    expect(tabs).not.toMatch(
      /title:\s*["'](?:dashboard|productivity|assistant-chat|projects\/index)["']/i,
    );
  ...
  test("superfícies sociais e de progresso não fabricam atividade competitiva", () => {
    const social = [
      source("../app/(app)/arena.tsx"),
      source("../app/(app)/community/index.tsx"),
      source("../app/(app)/community/[channelId].tsx"),
      source("../app/(app)/community/squads/[squadId].tsx"),
    ].join("\n");
    expect(social).not.toMatch(/fake user|usuário fake|dados? demo|ranking global|pessoas online/i);
    expect(source("../app/(app)/(tabs)/dashboard.tsx")).not.toMatch(
      /score de produtividade|pontuação de produtividade/i,
    );
  ...
  test("billing de teste permanece indisponível e sem CTA incompatível", () => {
    ...
    expect(premium).toContain('copyKey="legacy.3119000e25c3"');
  ...
  test("erros técnicos não chegam às superfícies principais", () => {
    expect([more, settings, source("../app/(app)/(tabs)/dashboard.tsx")].join("\n")).not.toMatch(
      /PostgREST|stack trace|HTTP payload|RPC\s|SQL\s|Supabase/i,
    );
  });
  ...
  test("notificações distinguem teste local de solicitação remota", () => {
    expect(settings).toContain('copyKey="legacy.df84bf81e8e3"');
    expect(settings).toContain('copyKey="legacy.51ec3c7f0d77"');
    expect(settings).toContain('copyKey="legacy.ece539f8f411"');
    expect(settings).toContain('copyKey="legacy.cdaf0be3fdd8"');
  ...
  test("a revisão permanece limitada ao código nativo permitido", () => {
    ...
    const revision13Range =
      "ddd1dd2fe8b13e7a219238c5cff900976ccc85aa...197d91bd3d131c71522c6162829bb2610e0ebd49";
    const changed = commandPaths(
      execFileSync("git", ["diff", "--name-only", revision13Range], {
        cwd: root,
        encoding: "utf8",
      }),
    );
