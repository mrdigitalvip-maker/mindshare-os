...
describe("atomic Free limits", () => {
  ...
  test("protects task creation and reopening", () => {
    ...
    expect(sql).toContain(":free-limit:tasks");
    expect(sql).toContain("if amount >= 30 then");
  });
  test("covers updates and reassociation", () => {
    expect(sql.match(/before insert or update of status, user_id/g)).toHaveLength(3);
    expect(sql).toContain(
      "before insert or update of completed, execution_status, user_id on public.tasks",
    );
  });
});
describe("task state invariant", () => {
  ...
  test("maps stable backend failures", () => {
    ...
    ).toContain("30 tarefas em aberto");
    expect(workspaceMutationError({ message: "TASK_STATE_CONFLICT" }).message).toContain(
      "estado da tarefa",
    );
  });
});
