...
describe("NXR-027 canonical user display name", () => {
  test("persisted profile full_name is canonical and whitespace is normalized", () => {
    expect(resolveCanonicalDisplayName("  Bruno   Silva  ", { full_name: "Auth Name" })).toBe("Bruno Silva");
  ...
  test("email and email local-part never become a personal name", () => {
    ...
    expect(resolveCanonicalDisplayName("user@example.com", { name: "other@example.com" })).toBeNull();
  ...
  test("Home uses canonical identity and has an honest neutral greeting", () => {
    ...
    expect(source("app/(app)/(tabs)/dashboard.tsx")).toContain("homeGreeting(profile.data?.displayName)");
  });

  test("Assistant context accepts a human name but rejects an email identity", () => {
    expect(JSON.parse(boundWorkspaceContext({ profile: " Bruno ", tasks: [], projects: [], studies: [] })).profile).toBe("Bruno");
    expect(JSON.parse(boundWorkspaceContext({ profile: "user@example.com", tasks: [], projects: [], studies: [] })).profile).toBeNull();
    expect(source("../supabase/functions/ai-chat/index.ts")).toContain("resolveCanonicalDisplayName(profile.data?.full_name, authMetadata)");
  });

  test("account email remains explicit account information, never a name fallback", () => {
    expect(source("app/(app)/settings.tsx")).toContain("<Text style={s.email}>{session?.user.email");
