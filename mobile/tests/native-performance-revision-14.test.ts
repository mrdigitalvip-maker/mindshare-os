...
import { hasActiveOfficialMembership, type CommunityMessage, type OfficialChannel } from "../lib/community";
import { projectQueryOptions, prefetchProject, type ProjectLoader } from "../lib/project-query";
import { queryKeys, taskMutationInvalidations, verifiedExecutionInvalidations } from "../lib/query-keys";
...
const workspace: ProjectWorkspace = {
  ...
  tasks: [], checkIns: [], tasksUnavailable: false, checkInsUnavailable: false,
};
...
describe("NXR-031 Project cache lifecycle", () => {
  ...
  test("list/detail identity is canonical and IDs are normalized", () => {
    ...
    expect(projectQueryOptions("user-1", " project-1 ", async () => workspace).queryKey).toEqual(queryKeys.project("project-1"));
  });

  test("prefetch and destination observers deduplicate one identical request", async () => {
    const qc = client(); let calls = 0; let resolve!: (value: ProjectWorkspace) => void;
    const loader: ProjectLoader = () => { calls += 1; return new Promise((done) => (resolve = done)); };
    ...
    expect(calls).toBe(1); resolve(workspace);
    await Promise.all([prefetched, destination]); expect(calls).toBe(1);
  });

  test("prefetch starts without blocking navigation", async () => {
    const qc = client(); let resolve!: (value: ProjectWorkspace) => void;
    const pending = prefetchProject(qc, "user-1", "project-1", () => new Promise((done) => (resolve = done)));
    ...
    resolve(workspace); await pending;
  });
...
describe("NXR-031 Community reconciliation", () => {
  const active = { id: "channel-1", joined: true, eligible: true, membershipStatus: "active" } as OfficialChannel;
  ...
  test("existing messages survive background reconciliation failures", async () => {
    const qc = client(); const key = queryKeys.communityMessages("channel-1");
    const cached = { pages: [[message]], pageParams: [undefined] }; qc.setQueryData(key, cached);
    await qc.fetchQuery({ queryKey: key, queryFn: async () => { throw new Error("offline"); } }).catch(() => undefined);
  ...
  test("realtime invalidation does not fabricate messages", () => {
    expect(communityHooks).toContain("invalidateQueries({ queryKey: queryKeys.communityMessages(channelId) })");
...
describe("NXR-031 request and invalidation boundaries", () => {
  test("independently safe authoritative project reads start in parallel", () => {
    expect(workspaceService).toContain("const [projectResult, tasksResult, checkInsResult] = await Promise.allSettled([");
  ...
  test("ordinary mutations stay targeted while verified execution remains complete", () => {
    ...
    for (const required of [queryKeys.tasks, queryKeys.journeys, queryKeys.momentum, queryKeys.arena])
