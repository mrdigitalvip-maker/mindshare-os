...
export type ProfileIdentity = ProfileRecord & {
...
export function normalizeProfileIdentity(
  ...
  return {
    ...profile,
    displayName: resolveCanonicalDisplayName(profile.fullName, user.user_metadata),
...
export function initialProfileValues(user: Pick<User, "id" | "user_metadata" | "app_metadata">) {
  ...
  return {
    id: user.id,
    full_name: normalizeHumanName(identity.name),
