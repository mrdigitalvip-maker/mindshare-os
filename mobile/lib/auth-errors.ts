...
export function presentAuthError(
  error: unknown,
  fallback: AuthErrorCategory = "UNKNOWN",
): PresentedAuthError {
  ...
  if (value.includes("password") && (value.includes("weak") || value.includes("short")))
    return {
      category: "VALIDATION",
      message: "Crie uma senha mais forte, com pelo menos 8 caracteres.",
    };
  ...
  if (
    value.includes("invalid_callback") ||
    value.includes("invalid_redirect") ||
    value.includes("invalid otp")
  )
    return {
      category: "CALLBACK",
      message: "Este link de autenticação é inválido. Solicite um novo link.",
    };
  const messages: Record<AuthErrorCategory, string> = {
    VALIDATION: "Revise os dados informados.",
    INVALID_CREDENTIALS: "Não foi possível entrar.",
    EMAIL_NOT_CONFIRMED: "Confirme seu e-mail antes de entrar.",
    AUTH_PROVIDER: "Não foi possível entrar com Google. Tente novamente.",
    AUTH_CANCELLED: "A autenticação foi cancelada.",
    CALLBACK: "Não foi possível concluir a autenticação. Tente entrar novamente.",
    ...
    PROFILE: "Não foi possível preparar seu perfil.",
    CONFIGURATION: "A autenticação está temporariamente indisponível.",
