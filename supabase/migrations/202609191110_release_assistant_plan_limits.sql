-- Final release audit: keep Assistant quotas aligned with the published Free/Premium plans.
-- Free: 10 messages/day, 2 attachment analyses/day.
-- Premium (paid or 90-day reward): 100 messages/day, 20 attachment analyses/day.
-- Internal full-access overrides remain unlimited for owner/test operations only.
...
begin
  ...
  internal_full := public.has_internal_full_access(uid);

  select * into prior
    from public.assistant_usage_claims
    where user_id=uid and request_id=p_request_id;
  if found then
    if prior.request_fingerprint <> p_request_fingerprint or prior.has_attachment <> p_has_attachment then
      raise exception using errcode='P0001', message='request_id_conflict';
    end if;

    assistant_cap := case
      when internal_full then 2147483647
      when prior.entitlement='premium' then 100
      else 10
    end;
    attachment_cap := case
      when internal_full then 2147483647
      when prior.entitlement='premium' then 20
      else 2
    end;
    ...
    return jsonb_build_object(
      'allowed',true,
      'replay',true,
      'entitlement',case when internal_full then 'premium' else prior.entitlement end,
      'unlimited',internal_full,
  ...
  entitlement_name := case when premium or internal_full then 'premium' else 'free' end;
  assistant_cap := case
    when internal_full then 2147483647
  ...
  if not internal_full and assistant_used >= assistant_cap then
    return jsonb_build_object(
      'allowed',false,
      'replay',false,
      'deniedFeature','assistant_standard',
  ...
  if not internal_full and p_has_attachment and attachment_used >= attachment_cap then
    return jsonb_build_object(
      'allowed',false,
      'replay',false,
      'deniedFeature','assistant_attachment',
      'entitlement',entitlement_name,
      'unlimited',false,
      'assistant',jsonb_build_object('used',assistant_used,'limit',assistant_cap),
  ...
  insert into public.assistant_usage_claims(
    ...
    uid,p_request_id,p_request_fingerprint,p_has_attachment,entitlement_name,today
  );
