...
const studio=read("supabase/migrations/202609180021_e43_studio_rls_indexes.sql");
const passport=read("supabase/migrations/202609180022_e44_passport_companion_rls_indexes.sql");

test("E43 hardens Studio owner policies without changing authenticated CRUD surface",()=>{
  for(const table of [
...
test("E43 adds only the missing Studio leading FK indexes",()=>{
  for(const name of [
...
test("E44 preserves exact Passport companion operation surface and authenticated role",()=>{
  for(const name of [
...
test("E44 adds only missing Passport companion leading FK indexes",()=>{
  for(const name of [
